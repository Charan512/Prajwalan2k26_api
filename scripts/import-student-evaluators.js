...
const updateEvaluators = async () => {
    try {
        ...
        // First, delete all existing student evaluators to ensure a clean slate
        const deleteResult = await User.deleteMany({ role: 'evaluator', evaluatorType: 'student' });
        console.log(`Cleared ${deleteResult.deletedCount} existing student evaluators.`);

        const csvContent = fs.readFileSync(path.join(__dirname, '../../studentEvalutors.csv'), 'utf-8');
        ...
        console.log(`Successfully created ${insertedUsers.length} student evaluators:`);
